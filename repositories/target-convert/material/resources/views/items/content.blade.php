<div x-data="materials_index_items()">
    @include('materials::items.content.' . $type_str)
    {{ $items->links() }}
    <!-- Edit Modal -->
                    ...
                    <div class="sm:flex sm:items-start">
                        ...
                        <div class="mt-3 w-full text-center sm:mt-0 sm:ml-4 sm:text-left space-y-4">
                            ...
                            <div class="relative z-0 mb-6 w-full group px-6">
                                <input x-model="title" autocomplete="off" type="text" name="title" class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none focus:outline-none focus:ring-0 focus:border-main-600 peer" placeholder=" " required />
                                <label for="title" class="absolute text-sm text-gray-500 duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:text-main-600 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6">{{ __('Material Title') }}</label>
                            </div>
                            <div class="relative px-6 space-y-4">
                                ...
                                <template x-for="keyword in keywords" :key="'tag_' + keyword">
                                    <div class="flex items-center">
                                        <input :id="'checkbox-' + keyword" x-model="checkboxes[keyword]" name="tags[]" :value="keyword" :aria-describedby="'checkbox-' + keyword" type="checkbox" class="w-4 h-4 text-main-600 bg-gray-100 rounded border-gray-300 ">
                                        <label :for="'checkbox-' + keyword" x-text="keyword" class="ml-3 text-sm font-medium text-gray-900"></label>
                                    </div>
                                </template>
                            </div>
                        </div>
    ...
    function materials_index_items() {
        return {
            ...
            drop(item) {
                ...
                this.drop_url = '{{ route('material.items.destroy', 'ref-id') }}'.replace('ref-id', item.id);
            },
            restore(item) {
                axios.post('{{ route('material.items.restore', 'ref-id') }}'.replace('ref-id', item.id), {
                    _method: 'PUT'
                })
            ...
            edit(item) {
                ...
                this.update_url = '{{ route('material.items.update', 'ref-id') }}'.replace('ref-id', item.id);
            },
            reset() {
                this.search = null,
                this.keywords = [],
